...
def func_1(x):
    ...
    return 0.5 * tf.reduce_sum(A * tf.square(x) + 0.2 * tf.math.exp(x), axis=-1)


class LinalgLinearOperatorApproxHessian(tf.test.TestCase):
    def setUp(self):
        pass

    def test_linalg_linear_operator_approx_hessian_do_dense_1(self):
        shape = (16, 5)
    ...
    def test_linalg_linear_operator_approx_hessian_matmul_1(self):
        ...
        self.assertAllClose(Hess.matmul(v0), tf.linalg.matmul(Hess.to_dense(), v0), atol=1e-3)

    def test_linalg_linear_operator_approx_hessian_matvec_1(self):
        shape = (16, 5)

        x0 = tf.random.normal(shape=shape, dtype=tf.float32)
        v0 = tf.random.normal(shape=shape, dtype=tf.float32)

        Hess = nnu.linalg.LinearOperatorApproxHessian(func_1, x0, epsilon = 1e-2, use_pfor=True)

        self.assertAllClose(Hess.matvec(v0), tf.linalg.matvec(Hess.to_dense(), v0), atol=1e-3)

    def test_linalg_linear_operator_approx_hessian_diag_part_1(self):
        shape = (16, 5)
    ...
    def test_linalg_linear_operator_approx_hessian_trace_1(self):
        shape = (16, 5)
